...
export const saveProduct = (name, price, amount) =>
  db
    .prepare("INSERT INTO product (name, price, amount) VALUES(?,?,?)")
    .run(name, price, amount);

export const updateProduct = (id, name, price, amount) =>
  db
    .prepare("UPDATE product SET name = ?, price = ?, amount = ? WHERE id=?")
    .run(name, price, amount, id);
